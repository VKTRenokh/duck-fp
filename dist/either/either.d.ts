...
export type FoldFunction<Left, Right> = <R>(lfn: (e: Left) => R, rfn: (v: Right) => R) => R;
export type MapFunction<Left, Right> = <R>(fn: (v: Right) => R) => Either<Left, R>;
export type ApFunction<Left, Right> = <R>(fn: Either<Left, (v: Right) => R>) => Either<Left, R>;
export type MapLeft<Left, Right> = <R>(f: (v: Left) => R) => Either<R, Right>;
export type FlatMapFunction<Left, Right> = <R>(fn: (v: Right) => Either<Left, R>) => Either<Left, R>;
export interface EnsureOrElse<L, R> {
    <T extends R>(p: Refinement<R, T>, c: (v: R) => L): Either<L, T>;
    (p: Predicate<R>, c: (v: R) => L): Either<L, R>;
}
export type MergeFunction<Left, Right> = <Nr>(or: Either<Left, Nr>) => Either<Left, Merged<Right, Nr>>;
export type AsyncMapFunction<Left, Right> = <R>(fn: (v: Right) => Promise<R>) => TaskEither<Left, R>;
/**
 ...
 */
export declare const left: <L, R = never>(e: L) => Either<L, R>;
/**
 ...
 */
export declare const right: <R, L = never>(v: R) => Either<L, R>;
/**
 * Alias for E.right.
 * @see {@link right}
 * @example
 ...
 */
export declare const of: <Right, Left>(value: Right) => Either<Left, Right>;
export type GetRight<T extends Either<unknown, unknown>> = T extends Either<infer _, infer U> ? U : never;
export type GetLeft<T extends Either<unknown, unknown>> = T extends Either<infer U, infer _> ? U : never;
